...
import 'package:maktampos/ui-admin/extensions.dart';
...
class CreateMaterialDialog extends StatefulWidget {
  const CreateMaterialDialog({Key? key, this.productItem, required this.isCup}) : super(key: key);

  final MaterialItem? productItem;
  final bool isCup;

  @override
  State<CreateMaterialDialog> createState() => _CreateProductDialogState();
}

class _CreateProductDialogState extends State<CreateMaterialDialog> {
            ...
            Container(
              child: _createProductLoading ? ProgressLoading() : FlatButton.icon(
                ...
                onPressed: () {
                  ...
                  var materialParam = MaterialItemParam(productName, widget.productItem?.id);
                  if(materialParam.isValid()){
                    if(widget.productItem != null){
                      bloc.add(UpdateMaterial(materialParam));
                    }else{
                      bloc.add(CreateMaterial(materialParam));
                    }
                  }
                },
  ...
  showDeleteDialog(BuildContext context) {
    ...
    Widget continueButton = FlatButton(
      ...
      onPressed:  () {
        ...
        if(productId != null){
          bloc.add(DeleteMaterial(productId));
        }
        Navigator.pop(context);
      },
    );
